...
data Stack = Lib | Hand | Ing | Rfp | Grv deriving Eq
-- I stos:       Library
-- II stos:      Hand
-- IIIa) stos:   In Game
-- IIIb) stos:   Removed from play
-- IIIc) stos:   Graveyard
type Card = String
type Token = String
...
player_alive pl = adversary_alive $ visible pl
...
move_card c p w f t = if w/=t && w/=f then get_cstack p w else
  case w==t of 
    False -> delete c (get_cstack p f)
    True -> insert c (get_cstack p t)
...
move_played c p w f t = if w/=f && w/=t then  get_pstack p w else
  case w==f of
    True -> delete (Card c) (get_pstack p f)
    False -> insert (Card c) (get_pstack p t)
